...
| Corpus governance | `cpp/include/cct/corpus.hpp`, `cpp/src/corpus.cpp` | Source policy, bounded PII/code hints, duplicate checks, split records, sharding, structured quarantine, and versioned serialized corpus state | Implemented and tested; raw PII is purged before storage, while the detector remains a bounded heuristic rather than a policy-grade classifier |
...
| Track 1 acquisition | `cpp/include/cct/track1.hpp`, `cpp/src/track1.cpp` | Pinned WikiText-2 archive and GEM SQuAD acquisition, bounded structural parsing, whole-file row counts, balancing, split isolation, atomic caches, and source attestations | Implemented and tested; parser and acquisition are hardened native paths, with provenance binding distinct from independent upstream equivalence |
...
The causal and memory APIs are typed rather than unstructured maps. The event store rejects duplicate IDs, validates dimensions, checks explicit unresolved parents, and rejects cycles. The memory layer supports append/update/tombstone/quarantine records and replays its event log. Corpus quarantine purges raw detected PII before storage and sanitizes legacy flagged records. Track 1 uses direct pinned files after the rows API rate-limit issue was discovered, and the native acquisition path now uses argument-vector processes, unique atomic temporary files, whole-file direct-source counts, cache digests, and source-attestation manifests.

...
The completed hardening set includes SHA-256 memory integrity, atomic memory/causal/knowledge/release publication, exact-span citation verification, all-eligible conflict preflight, bounded snapshot parsing, versioned external-embedding contracts, indexed lexical retrieval with linear correctness oracles, injected knowledge embedding-provider identity, checkpoint-backed inference, cooperative streaming cancellation, synchronized inference service state, bounded inference cache/state, release-to-checkpoint activation, durable repository-artifact black-box inference, self-contained strict CMake warnings, expanded warning compilation, const-correct Track 1 source construction, deterministic parser mutation coverage, and complete bounded ASan/UBSan gate coverage.

The following remain explicitly scoped research items rather than silently closed claims: semantic quality of learned embeddings beyond the versioned provider interface, broad capability evaluation beyond the declared checkpoint and gate contracts, human-preference equivalence, distributed deployment readiness, and general intelligence. The default memory and knowledge backends are deterministic/external-vector engineering baselines; their indexes and correctness oracles do not claim semantic superiority.

...
The **Chrono-Causal Tapestry — Adaptive Spectral Engine (CCT-ASE)** is a research prototype for testing causal event fields and efficient spectral dynamics. The repository currently provides a reproducible **native C++20 numerical, sequence, causal-event, persistent-memory, small-scale language/code, bounded deliberation, controlled multimodal, governed-corpus, tokenizer/representation, trainable next-token, CPU scaling, supervised-adaptation, preference-alignment, verified-retrieval, and production-like inference-operations substrate**; it does not claim to be a general language model or superintelligence system. The implemented Stage 0 through Stage 16 gates establish reproducibility, numerical correctness, recurrent trainability, efficiency measurement, leakage-controlled graph conditioning, intervention prediction, counterfactual evaluation, persistent checksummed memory, exact retrieval, provenance, deletion, conflict handling, poisoning resistance, checkpoint replay, bounded token learning, code-safety checks, independent verification, evidence abstention, deny-by-default offline tools, interruption/replay, typed multimodal events, temporal/spatial alignment, modality masking, cross-modal memory, deterministic simulation, transfer controls, audit logging, real-source corpus governance, byte/subword/hybrid tokenization, immutable tokenizer snapshots, source offsets, byte fallback, packed/padded causal batches, stable categorical next-token loss, analytic recurrent gradients, optimizer state, checkpoint resume, three-seed validation pilots, matched native controls, deterministic scaling curves, CPU reference/fused parity, resource profiling, atomic recovery, six-task supervised fine-tuning, target-span loss masks, parameter-efficient adapters, structured-output validation, grounded citation checks, safety retention, adapter permissions, deletion lineage, governed preference data, DPO-like preference optimization, verifier-weighted reranking, adversarial alignment controls, tenant-safe retrieval, freshness/version checks, citation span hashes, conflict surfacing, poisoning isolation, deletion replay, audit traces, versioned inference APIs, dynamic batching, streaming cancellation, state/cache isolation, SLO measurement, fault controls, canarying, and rollback on declared fixtures.

> **Current status:** Stages 0 through 16 are implemented in native C++ and pass their mandatory gates on declared fixtures. Stage 16 adds a production-like native serving binding with versioned schemas, policy enforcement, batching, streaming controls, state/cache isolation, observability, fault handling, canarying, and rollback. Stage 17 remains **specification-only**; the project does not claim internet-scale availability, public launch readiness, production language-model quality, or superintelligence.
